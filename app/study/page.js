            ...
            {set.questions?.map((q,qi)=>(
              <div key={qi} style={{background:'white',borderRadius:'12px',padding:'24px',marginBottom:'16px'}}>
                ...
                {q.options?.map((opt,oi)=>{
                  ...
                  if(submitted){if(opt===q.answer){bg='#d1fae5';border='1px solid #10b981';}else if(opt===answers[qi]){bg='#fee2e2';border='1px solid #ef4444';}}
                  else if(answers[qi]===opt){bg='#ede9fe';border='1px solid #6d28d9';}
                  return <div key={oi} onClick={()=>!submitted&&handleAnswer(qi,opt)} style={{padding:'12px',border,borderRadius:'8px',marginBottom:'8px',cursor:submitted?'default':'pointer',background:bg}}>{opt}</div>;
                })}
              </div>
            ))}
            {!submitted?<button onClick={handleSubmit} style={{width:'100%',padding:'14px',background:'#6d28d9',color:'white',border:'none',borderRadius:'8px',fontSize:'16px',cursor:'pointer'}}>Submit Quiz</button>
            :<div style={{background:'white',borderRadius:'12px',padding:'24px',textAlign:'center'}}><p style={{fontSize:'24px',fontWeight:'bold'}}>Score: {score}/{set.questions.length}</p><p style={{color:'#666',marginTop:'8px'}}>{score===set.questions.length?'🎉 Perfect!':score>=set.questions.length/2?'👍 Good job!':'💪 Keep studying!'}</p></div>}
          </div>
        )}
      </div>
    </div>
...
export default function Study() {
  return <Suspense fallback={<div style={{padding:'32px'}}>Loading...</div>}><StudyContent /></Suspense>;
}